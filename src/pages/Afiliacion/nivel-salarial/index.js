...
const endPoint = 'http://127.0.0.1:8000/show_nivel_salarial/'
...
const AppPage = ({}) => {
  ...
  const handleSi = () => {
    if (deleteRow(registroSeleccionado.id, '/update_nivel_salarial/')) {
      toast.success(t('Record deleted successfully!'))
      router.push('./nivel-salarial')
    } else {
  ...
  const onSubmit = data => {
    if (saveRow(data, '/store_nivel_salarial/')) {
  ...
  const onSubmitEdit = e => {
    e.preventDefault()
    if (saveRow(registroSeleccionado, '/store_nivel_salarial/')) {
