...
const endPoint = 'http://127.0.0.1:8000/show_paciente_tipo/'
...
const AppPage = ({}) => {
  ...
  const handleSi = () => {
    if (deleteRow(registroSeleccionado.id, '/update_paciente_tipo/')) {
      toast.success(t('Record deleted successfully!'))
      router.push('./tipo-paciente')
    } else {
  ...
  const onSubmit = data => {
    if (saveRow(data, '/store_paciente_tipo/')) {
  ...
  const onSubmitEdit = e => {
    e.preventDefault()
    if (saveRow(registroSeleccionado, '/store_paciente_tipo/')) {
