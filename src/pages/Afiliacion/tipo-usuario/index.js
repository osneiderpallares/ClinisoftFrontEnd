...
const endPoint = 'http://127.0.0.1:8000/show_tipo_usuario/'
const endPoint_homologodian = 'http://127.0.0.1:8000/show_tributos_fe/'
...
const AppPage = ({}) => {
  ...
  const columns = [
    ...
    },
    {
        flex: 0.25,
        minWidth: 230,
        field: 'cac_abreviacion',
        headerName:'CAC_ABBREVIATION'
      },
    {
      flex: 0.15,
      minWidth: 120,
    ...
    },
    {
        ...
        minWidth: 230,
        field: 'rips_codigo',
        headerName:t('RIPS_CODE')
      },
      {
        flex: 0.25,
        minWidth: 230,
        field: 'nombre_homologo_dian',
        headerName:'HOMOLOGO_DIAN'
      },
    {
      flex: 0.125,
      minWidth: 140,
  ...
  const [homologo_dianRows, sethomologo_dianRows] = useState(null)

  const peticionGet = async () => {
    await axios.get(endPoint).then(response => {
      setRows(response.data)
    })
    await axios.get(endPoint_homologodian).then(response => {
      sethomologo_dianRows(response.data)
    })
  }
  ...
  const handleSi = () => {
    if (deleteRow(registroSeleccionado.id, '/update_tipo_usuario/')) {
  ...
  const schema = yup.object().shape({
    ...
    nombre: yup
      ...
      cac_abreviacion: yup
      .string()
      .min(1, obj => showErrors(t('cac_abbreviation'), obj.value.length, obj.min))
      .required(),
      rips_codigo: yup
      .number()
      .min(1, obj => showErrors(t('rips_code'), obj.value.length, obj.min))
      .required(),
    estado: yup.string()
  })
  ...
  const onSubmit = data => {
    if (saveRow(data, '/store_tipo_usuario/')) {
  ...
  const onSubmitEdit = e => {
    e.preventDefault()
    if (saveRow(registroSeleccionado, '/store_tipo_usuario/')) {
                ...
                <Controller
                  ...
                  render={({ field: { value, onChange } }) => (
                    <CustomTextField
                      fullWidth
                      value={value}
                      label={t('CAC_Abbreviation')}
                      inputProps={{ maxLength: 50 }}
                      onChange={onChange}
                      placeholder={t('Enter the cac abbreviation')}
                      error={Boolean(errors.cac_abreviacion)}
                      aria-describedby='validation-schema-cac_abbreviation'
                      {...(errors.abreviacion && { helperText: errors.cac_abreviacion.message })}
                    />
                  )}
                />
              </Grid>
                ...
                <Controller
                  ...
                  render={({ field: { value, onChange } }) => (
                    <CustomTextField
                      ...
                      error={Boolean(errors.rips_codigo)}
                      aria-describedby='validation-schema-rips_code'
                      {...(errors.rips_codigo && { helperText: t('rips codigo') })}
                    />
                  )}
                />
              </Grid>
                ...
                <Controller
                  ...
                  render={({ field: { value, onChange } }) => (
                    <CustomTextField
                      ...
                      // inputProps={{ maxLength: 50 }}
                      // onChange={onChange}
                      // placeholder={t('Enter the homologo_dian')}
                      // error={Boolean(errors.abreviacion)}
                      // aria-describedby='validation-schema-homologo_dian'
                      // {...(errors.abreviacion && { helperText: errors.abreviacion.message })}
                      >
                      {homologo_dianRows?.map(hom => {
                        return (
                          <MenuItem key={hom.id} value={hom.id}>
                            {hom.nombre}
                          </MenuItem>
                        )
                      })}
            ...
            <Grid container spacing={6}>
              <input
                ...
                onChange={e => {
                  setRegistroSeleccionado({
                    ...
                    rips_codigo: registroSeleccionado.rips_codigo,
                    homologo_dian:registroSeleccionado.homologo_dian,
              ...
              <Grid item sm={6} xs={6}>
                <CustomTextField
                  ...
                  required
                  name='cac_abreviacion'
                  value={registroSeleccionado.cac_abreviacion}
                  label={t('CAC_Abbreviation')}
                  inputProps={{ maxLength: 50 }}
                  onChange={e => {
                    setRegistroSeleccionado({
                      ...
                      nombre: registroSeleccionado.nombre,
                      abreviacion: registroSeleccionado.abreviacion,
                      cac_abreviacion: e.target.value,
                      rips_codigo: registroSeleccionado.rips_codigo,
                      homologo_dian:registroSeleccionado.homologo_dian,
                      estado: registroSeleccionado.estado
                    })
              ...
              <Grid item sm={6} xs={6}>
                <CustomTextField
                  ...
                  required
                  name='rips_codigo'
                  value={registroSeleccionado.rips_codigo}
                  label={t('Rips_Code')}
                  inputProps={{ maxLength: 50 }}
                  onChange={e => {
                    setRegistroSeleccionado({
              ...
              <Grid item sm={6} xs={6}>
                <CustomTextField
                  ...
                  SelectProps={{
                    MenuProps,
                    value:registroSeleccionado.homologo_dian,
                    onChange:e => {
                      setRegistroSeleccionado({
                        id: registroSeleccionado.id,
                        nombre: registroSeleccionado.nombre,
                        abreviacion: registroSeleccionado.abreviacion,
                        cac_abreviacion:registroSeleccionado.cac_abreviacion,
                        rips_codigo:registroSeleccionado.homologo_dian,
                        homologo_dian: e.target.value,
                        estado: registroSeleccionado.estado
                      })
                     }
                    }}
