...
const endPoint = 'http://127.0.0.1:8000/show_agendas_grupos/'
...
const AppPage = ({}) => {
  ...
  const handleSi = () => {
    if (deleteRow(registroSeleccionado.id, '/update_agendas_grupos/')) {
      toast.success(t('Record deleted successfully!'))
      router.push('./grupo-agenda')
    } else {
  ...
  const onSubmit = data => {
    if (saveRow(data, '/store_agendas_grupos/')) {
  ...
  const onSubmitEdit = e => {
    e.preventDefault()
    if (saveRow(registroSeleccionado, '/store_agendas_grupos/')) {
  ...
  const table = (
    <DataGrid
      ...
      columnVisibilityModel={{ id: true, create_up: true }}
      slotProps={{
        baseButton: {
          size: 'medium',
          variant: 'outlined'
