...
const endPoint = 'http://127.0.0.1:8000/show_cac_base_dx_diferenciacion/'
...
const AppPage = ({}) => {
  ...
  const handleSi = () => {
    if (deleteRow(registroSeleccionado.id, '/update_cac_base_dx_diferenciacion/')) {
      toast.success(t('Record deleted successfully!'))
      router.push('./bases-dx-diferenciaciones')
    } else {
  ...
  const onSubmit = data => {
    if (saveRow(data, '/store_cac_base_dx_diferenciacion/')) {
  ...
  const onSubmitEdit = e => {
    e.preventDefault()
    if (saveRow(registroSeleccionado, '/store_cac_base_dx_diferenciacion/')) {
