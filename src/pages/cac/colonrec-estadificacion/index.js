...
const endPoint = 'http://127.0.0.1:8000/show_cac_colorrec_estadif/'
...
const AppPage = ({}) => {
  ...
  const handleSi = () => {
    if (deleteRow(registroSeleccionado.id, '/update_cac_colorrec_estadif/')) {
      toast.success(t('Record deleted successfully!'))
      router.push('./colonrec-estadificacion')
    } else {
  ...
  const onSubmit = data => {
    ...
    if (saveRow(data, '/store_cac_colorrec_estadif/')) {
  ...
  const onSubmitEdit = e => {
    e.preventDefault()
    if (saveRow(registroSeleccionado, '/store_cac_colorrec_estadif/')) {
