...
const endPoint = 'http://127.0.0.1:8000/show_cac_estados_fin_tto/'
...
const AppPage = ({}) => {
  ...
  const handleSi = () => {
    if (deleteRow(registroSeleccionado.id, '/update_cac_estados_fin_tto/')) {
      toast.success(t('Record deleted successfully!'))
      router.push('./estado-fin-tto')
    } else {
  ...
  const onSubmit = data => {
    ...
    if (saveRow(data, '/store_cac_estados_fin_tto/')) {
  ...
  const onSubmitEdit = e => {
    e.preventDefault()
    if (saveRow(registroSeleccionado, '/store_cac_estados_fin_tto/')) {
  ...
  return (
    <Card>
      <CardHeader
        title={t('End of Treatment Status')}
        action={
          <Tooltip title={t('Add')}>
