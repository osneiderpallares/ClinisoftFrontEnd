...
const endPoint = 'http://127.0.0.1:8000/show_her2_resultados/'
...
const AppPage = ({}) => {
  ...
  const handleSi = () => {
    if (deleteRow(registroSeleccionado.id, '/update_her2_resultados/')) {
      toast.success(t('Record deleted successfully!'))
      router.push('./her2-resultados')
    } else {
  ...
  const onSubmit = data => {
    ...
    if (saveRow(data, '/store_her2_resultados/')) {
  ...
  const onSubmitEdit = e => {
    e.preventDefault()
    if (saveRow(registroSeleccionado, '/store_her2_resultados/')) {
