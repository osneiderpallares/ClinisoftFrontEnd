...
const endPoint = 'http://127.0.0.1:8000/show_cac_leucemias_clasif/'
...
const AppPage = ({}) => {
  ...
  const handleSi = () => {
    if (deleteRow(registroSeleccionado.id, '/update_cac_leucemias_clasif/')) {
      toast.success(t('Record deleted successfully!'))
      router.push('./leucemia-clasificada')
    } else {
  ...
  const onSubmit = data => {
    ...
    if (saveRow(data, '/store_cac_leucemias_clasif/')) {
  ...
  const onSubmitEdit = e => {
    e.preventDefault()
    if (saveRow(registroSeleccionado, '/store_cac_leucemias_clasif/')) {
