...
const endPoint = 'http://127.0.0.1:8000/show_motivo_histologia/'
...
const AppPage = ({}) => {
  ...
  const handleSi = () => {
    if (deleteRow(registroSeleccionado.id, '/update_motivo_histologia/')) {
      toast.success(t('Record deleted successfully!'))
      router.push('./motivos-histologia')
    } else {
  ...
  const onSubmit = data => {
    if (saveRow(data, '/store_motivo_histologia/')) {
  ...
  const onSubmitEdit = e => {
    e.preventDefault()
    if (saveRow(registroSeleccionado, '/store_motivo_histologia/')) {
