...
const endPoint = 'http://127.0.0.1:8000/show_cac_objetivo_tratamiento/'
...
const AppPage = ({}) => {
  ...
  const handleSi = () => {
    if (deleteRow(registroSeleccionado.id, '/update_cac_objetivo_tratamiento/')) {
      toast.success(t('Record deleted successfully!'))
      router.push('./objetivos-de-tratamientos')
    } else {
  ...
  const onSubmit = data => {
    if (saveRow(data, '/store_cac_objetivo_tratamiento/')) {
  ...
  const onSubmitEdit = e => {
    e.preventDefault()
    if (saveRow(registroSeleccionado, '/store_cac_objetivo_tratamiento/')) {
