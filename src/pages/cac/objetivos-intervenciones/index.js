...
const endPoint = 'http://127.0.0.1:8000/show_cac_objetivo_intervencion/'
...
const AppPage = ({}) => {
  ...
  const handleSi = () => {
    if (deleteRow(registroSeleccionado.id, '/update_cac_objetivo_intervencion/')) {
      toast.success(t('Record deleted successfully!'))
      router.push('./objetivos-intervenciones')
    } else {
  ...
  const onSubmit = data => {
    ...
    if (saveRow(data, '/store_cac_objetivo_intervencion/')) {
  ...
  const onSubmitEdit = e => {
    e.preventDefault()
    if (saveRow(registroSeleccionado, '/store_cac_objetivo_intervencion/')) {
  ...
  return (
    <Card>
      <CardHeader
        title={t('Intervention Objectives')}
        action={
          <Tooltip title={t('Add')}>
