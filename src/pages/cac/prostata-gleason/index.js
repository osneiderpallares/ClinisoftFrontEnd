...
const endPoint = 'http://127.0.0.1:8000/show_cac_prostatas_gleason/'
...
const AppPage = ({}) => {
  ...
  const handleSi = () => {
    if (deleteRow(registroSeleccionado.id, '/update_cac_prostatas_gleason/')) {
      toast.success(t('Record deleted successfully!'))
      router.push('./prostata-gleason')
    } else {
  ...
  const onSubmit = data => {
    ...
    if (saveRow(data, '/store_cac_prostatas_gleason/')) {
  ...
  const onSubmitEdit = e => {
    e.preventDefault()
    if (saveRow(registroSeleccionado, '/store_cac_prostatas_gleason/')) {
