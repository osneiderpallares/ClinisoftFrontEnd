...
const endPoint = 'http://127.0.0.1:8000/show_cac_qtx_fases/'
...
const AppPage = ({}) => {
  ...
  const handleSi = () => {
    if (deleteRow(registroSeleccionado.id, '/update_cac_qtx_fases/')) {
      toast.success(t('Record deleted successfully!'))
      router.push('./quimioterapia-fases')
    } else {
  ...
  const onSubmit = data => {
    ...
    if (saveRow(data, '/store_cac_qtx_fases/')) {
  ...
  const onSubmitEdit = e => {
    e.preventDefault()
    if (saveRow(registroSeleccionado, '/store_cac_qtx_fases/')) {
