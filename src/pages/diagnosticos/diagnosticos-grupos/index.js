...
const endPoint = 'http://127.0.0.1:8000/show_diagnostico_grupo/'
const endPoint_cacdiagnostico = 'http://127.0.0.1:8000/show_cac_diagnostico/'
const endPoint_tipocodigo = 'http://127.0.0.1:8000/show_tipo_codigo/'
...
const AppPage = ({}) => {
  ...
  const columns = [
    ...
    },
    {
        flex: 0.25,
        minWidth: 230,
        field: 'nombre_tipo_codigo',
        headerName: t('Code Type')
    },
    {
      flex: 0.25,
      minWidth: 230,
    ...
    },
    {
        flex: 0.25,
        minWidth: 230,
        field: 'nombre_cac_diagnostico',
        headerName: t('CAC_Diagnostic')
    },
  ...
  const [tipocodigoRows, settipocodigoRows] = useState(null)
  const [cacdiagnosticoRows, setcacdiagnosticoRows] = useState(null)
  
  const peticionGet = async () => {
    await axios.get(endPoint).then(response => {
        setRows(response.data)
    })
    await axios.get(endPoint_tipocodigo).then(response => {
        settipocodigoRows(response.data)
    })
    await axios.get(endPoint_cacdiagnostico).then(response => {
        setcacdiagnosticoRows(response.data)
    })
  }
  ...
  const handleSi = () => {
    if (deleteRow(registroSeleccionado.id, '/update_diagnostico_grupo/')) {
      toast.success(t('Record deleted successfully!'))
      router.push('./diagnosticos-grupos')
    } else {
  ...
  const schema = yup.object().shape({
    ...
    tipo_codigo: yup
      .number()
      .min(1, obj => showErrors(t('code type'), obj.value.length, obj.min))
      .required(),
    codigo: yup
      .string()
      .min(1, obj => showErrors(t('code'), obj.value.length, obj.min))
      .required(),
    cac_diagnostico: yup
      .number()
      .min(1, obj => showErrors(t('cac_diagnostic'), obj.value.length, obj.min))
      .required(),
    estado: yup.string()
  })
  ...
  const onSubmit = data => {
    if (saveRow(data, '/store_diagnostico_grupo/')) {
  ...
  const onSubmitEdit = e => {
    e.preventDefault()
    if (saveRow(registroSeleccionado, '/store_diagnostico_grupo/')) {
                ...
                <Controller
                  ...
                  render={({ field: { value, onChange } }) => (
                    <CustomTextField
                      ...
                      error={Boolean(errors.tipo_codigo)}
                      aria-describedby='validation-schema-code type'
                      {...(errors.tipo_codigo && { helperText: errors.tipo_codigo.message })}
                    >
                      {tipocodigoRows?.map(dg => {
                        return (
                ...
                <Controller
                  ...
                  render={({ field: { value, onChange } }) => (
                    <CustomTextField
                      ...
                      {...(errors.codigo && { helperText: errors.codigo.message })}
                    />
                  )}
                />
              </Grid>
                ...
                <Controller
                  ...
                  render={({ field: { value, onChange } }) => (
                    <CustomTextField
                      ...
                      fullWidth
                      value={value}
                      label={t('CAC_Diagnostic')}
                      SelectProps={{
                        MenuProps,
                        value: value,
                        onChange:onChange
                      }}
                      error={Boolean(errors.cac_diagnostico)}
                      aria-describedby='validation-schema-cac_diagnostic'
                      {...(errors.cac_diagnostico && { helperText: errors.cac_diagnostico.message })}
                    >
                      {cacdiagnosticoRows?.map(cac_dg => {
                        return (
                          <MenuItem key={cac_dg.id} value={cac_dg.id}>
                            {cac_dg.nombre}
                          </MenuItem>
                        )
                      })}
            ...
            <Grid container spacing={6}>
              <input
                ...
                onChange={e => {
                  setRegistroSeleccionado({
                    ...
                    cac_diagnostico: registroSeleccionado.cac_diagnostico,
                    tipo_codigo: registroSeleccionado.tipo_codigo,
              ...
              <Grid item sm={6} xs={6}>
                <CustomTextField
                  ...
                  SelectProps={{
                    MenuProps,
                    value:registroSeleccionado.tipo_codigo,
                    onChange: e => {
                      setRegistroSeleccionado({
                        id: registroSeleccionado.id,
                        nombre: registroSeleccionado.nombre,
              ...
              <Grid item sm={6} xs={6}>
                <CustomTextField
                  ...
                  required
                  name='codigo'
                  value={registroSeleccionado.codigo}
                  label={t('Code')}
                  inputProps={{ maxLength: 50 }}
                  onChange={e => {
                    setRegistroSeleccionado({
              ...
              <Grid item sm={6} xs={6}>
                <CustomTextField
                  ...
                  SelectProps={{
                    MenuProps,
                    value:registroSeleccionado.cac_diagnostico,
                    onChange: e => {
                      setRegistroSeleccionado({
                        id: registroSeleccionado.id,
                        nombre: registroSeleccionado.nombre,
                        codigo: registroSeleccionado.codigo,
                        cac_diagnostico: e.target.value,
                        tipo_codigo: registroSeleccionado.tipo_codigo,
                        estado: registroSeleccionado.estado
                    })
                   }
