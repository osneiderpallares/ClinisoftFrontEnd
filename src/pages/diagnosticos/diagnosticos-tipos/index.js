...
const endPoint = 'http://127.0.0.1:8000/show_diagnostico_tipo/'
...
const AppPage = ({}) => {
  ...
  const handleSi = () => {
    if (deleteRow(registroSeleccionado.id, '/update_diagnostico_tipo/')) {
      toast.success(t('Record deleted successfully!'))
      router.push('./diagnosticos-tipos')
    } else {
  ...
  const onSubmit = data => {
    if (saveRow(data, '/store_diagnostico_tipo/')) {
  ...
  const onSubmitEdit = e => {
    e.preventDefault()
    if (saveRow(registroSeleccionado, '/store_diagnostico_tipo/')) {
                ...
                <Controller
                  ...
                  render={({ field: { value, onChange } }) => (
                    <CustomTextField
                      ...
                      onChange={onChange}
                      placeholder={t('Enter the abbreviatura')}
                      error={Boolean(errors.abreviatura)}
                      aria-describedby='validation-schema-abbreviatura'
