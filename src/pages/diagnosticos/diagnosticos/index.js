...
const endPoint = 'http://127.0.0.1:8000/show_diagnostico/'
const endPoint_diagnosticogrupo = 'http://127.0.0.1:8000/show_diagnostico_grupo/'
...
const AppPage = ({}) => {
  ...
  const columns = [
    ...
    },
    {
        flex: 0.25,
        minWidth: 230,
        field: 'nombre_diagnostico_grupo',
        headerName: t('Diagnostic Groups')
    },
    {
        flex: 0.25,
        minWidth: 230,
  ...
  const [diagnosticogrupoRows, setdiagnosticogrupoRows] = useState(null)
  const [tipocodigoRows, settipocodigoRows] = useState(null)
  const peticionGet = async () => {
    await axios.get(endPoint).then(response => {
        setRows(response.data)
    })
    await axios.get(endPoint_diagnosticogrupo).then(response => {
        setdiagnosticogrupoRows(response.data)
    })
  ...
  const handleSi = () => {
    if (deleteRow(registroSeleccionado.id, '/update_diagnostico/')) {
  ...
  const schema = yup.object().shape({
    ...
    codigo: yup
      ...
      .required(),
    diagnostico_grupo: yup
      .number()
      .min(1, obj => showErrors(t('diagnostic groups'), obj.value.length, obj.min))
      .required(),
    tipo_codigo: yup
      .number()
  ...
  const onSubmit = data => {
    if (saveRow(data, '/store_diagnostico/')) {
  ...
  const onSubmitEdit = e => {
    e.preventDefault()
    if (saveRow(registroSeleccionado, '/store_diagnostico/')) {
                ...
                <Controller
                  ...
                  render={({ field: { value, onChange } }) => (
                    <CustomTextField
                      ...
                      fullWidth
                      value={value}
                      label={t('Diagnostic Groups')}
                      SelectProps={{
                        MenuProps,
                        value: value,
                        onChange:onChange
                      }}
                      error={Boolean(errors.diagnostico_grupo)}
                      aria-describedby='validation-schema-diagnostic groups'
                      {...(errors.diagnostico_grupo && { helperText: errors.diagnostico_grupo.message })}
                    >
                      {diagnosticogrupoRows?.map(dg => {
                        return (
            ...
            <Grid container spacing={6}>
              <input
                ...
                onChange={e => {
                  setRegistroSeleccionado({
                    ...
                    diagnostico_grupo: registroSeleccionado.diagnostico_grupo,
              ...
              <Grid item sm={6} xs={6}>
                <CustomTextField
                  ...
                  SelectProps={{
                    MenuProps,
                    value:registroSeleccionado.diagnostico_grupo,
                    onChange: e => {
                      setRegistroSeleccionado({
                        id: registroSeleccionado.id,
                        nombre: registroSeleccionado.nombre,
                        codigo: registroSeleccionado.codigo,
                        diagnostico_grupo: e.target.value,
                        tipo_codigo: registroSeleccionado.tipo_codigo,
                        estado: registroSeleccionado.estado
                    })
                   }
