...
const endPoint = 'http://127.0.0.1:8000/show_forma_diagnostico/'
...
const AppPage = ({}) => {
  ...
  const handleSi = () => {
    if (deleteRow(registroSeleccionado.id, '/update_forma_diagnostico/')) {
      toast.success(t('Record deleted successfully!'))
      router.push('./forma-diagnostico')
    } else {
  ...
  const onSubmit = data => {
    if (saveRow(data, '/store_forma_diagnostico/')) {
  ...
  const onSubmitEdit = e => {
    e.preventDefault()
    if (saveRow(registroSeleccionado, '/store_forma_diagnostico/')) {
