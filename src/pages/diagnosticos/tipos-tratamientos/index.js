...
const endPoint = 'http://127.0.0.1:8000/show_tipo_tratamiento/'
...
const AppPage = ({}) => {
  ...
  const handleSi = () => {
    if (deleteRow(registroSeleccionado.id, '/update_tipo_tratamiento/')) {
      toast.success(t('Record deleted successfully!'))
      router.push('./tipos-tratamientos')
    } else {
  ...
  const onSubmit = data => {
    if (saveRow(data, '/store_tipo_tratamiento/')) {
  ...
  const onSubmitEdit = e => {
    e.preventDefault()
    if (saveRow(registroSeleccionado, '/store_tipo_tratamiento/')) {
