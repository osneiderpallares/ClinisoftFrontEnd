...
const endPoint = 'http://127.0.0.1:8000/show_incapacidad_tipo/'
...
const AppPage = ({}) => {
  ...
  const handleSi = () => {
    if (deleteRow(registroSeleccionado.id, '/update_incapacidad_tipo/')) {
      toast.success(t('Record deleted successfully!'))
      router.push('./incapacidades-tipo')
    } else {
  ...
  const onSubmit = data => {
    if (saveRow(data, '/store_incapacidad_tipo/')) {
  ...
  const onSubmitEdit = e => {
    e.preventDefault()
    if (saveRow(registroSeleccionado, '/store_incapacidad_tipo/')) {
