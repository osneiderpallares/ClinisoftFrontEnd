...
const endPoint = 'http://127.0.0.1:8000/show_indice_dt/'
const endPoint_indice = 'http://127.0.0.1:8000/show_indice/'
...
const AppPage = ({}) => {
  ...
  const [indicerows, setindiceRows] = useState(null)

  const peticionGet = async () => {
    await axios.get(endPoint).then(response => {
        setRows(response.data)
      })
      await axios.get(endPoint_indice).then(response => {
        setindiceRows(response.data)
      })
  }
  ...
  const handleSi = () => {
    if (deleteRow(registroSeleccionado.id, '/update_indice_dt/')) {
      toast.success(t('Record deleted successfully!'))
      router.push('./indices-calidad-vida')
    } else {
  ...
  const onSubmit = data => {
    if (saveRow(data, '/store_indice_dt/')) {
  ...
  const onSubmitEdit = e => {
    e.preventDefault()
    if (saveRow(registroSeleccionado, '/store_indice_dt/')) {
                ...
                <Controller
                  ...
                  render={({ field: { value, onChange } }) => (
                    <CustomTextField
                      ...
                      aria-describedby='validation-schema-indice'
