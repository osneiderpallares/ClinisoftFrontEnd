...
const endPoint = 'http://127.0.0.1:8000/show_causa_muerte/'
...
const AppPage = ({}) => {
  ...
  const handleSi = () => {
    if (deleteRow(registroSeleccionado.id, '/update_causa_muerte/')) {
      toast.success(t('Record deleted successfully!'))
      router.push('./causa-muerte')
    } else {
  ...
  const onSubmit = data => {
    if (saveRow(data, '/store_causa_muerte/')) {
  ...
  const onSubmitEdit = e => {
    e.preventDefault()
    if (saveRow(registroSeleccionado, '/store_causa_muerte/')) {
