...
const endPoint = 'http://127.0.0.1:8000/show_comunidades_etnicas/'
...
const AppPage = ({}) => {
  ...
  const handleSi = () => {
    if (deleteRow(registroSeleccionado.id, '/update_comunidad_etnica/')) {
      toast.success(t('Record deleted successfully!'))
      router.push('./comunidad-etnica')
    } else {
  ...
  const onSubmit = data => {
    if (saveRow(data, '/store_comunidad_etnica/')) {
  ...
  const onSubmitEdit = e => {
    e.preventDefault()
    if (saveRow(registroSeleccionado, '/store_comunidad_etnica/')) {
