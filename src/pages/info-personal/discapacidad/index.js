...
const endPoint = 'http://127.0.0.1:8000/show_discapacidad/'
...
const AppPage = ({}) => {
  ...
  const handleSi = () => {
    if (deleteRow(registroSeleccionado.id, '/update_discapacidad/')) {
      toast.success(t('Record deleted successfully!'))
      router.push('./discapacidad')
    } else {
  ...
  const onSubmit = data => {
    if (saveRow(data, '/store_discapacidad/')) {
  ...
  const onSubmitEdit = e => {
    e.preventDefault()
    if (saveRow(registroSeleccionado, '/store_discapacidad/')) {
