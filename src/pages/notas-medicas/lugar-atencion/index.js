...
const endPoint = 'http://127.0.0.1:8000/show_lugar_atencion/'
const endPoint_ubicacion = 'http://127.0.0.1:8000/show_ubicacion/'
...
const AppPage = ({}) => {
  ...
  const [ubicacionRows, setubicacionRows] = useState(null)
  const peticionGet = async () => {
    await axios.get(endPoint).then(response => {
      setRows(response.data)
    })
    await axios.get(endPoint_ubicacion).then(response => {
      setubicacionRows(response.data)
    })
  }
  ...
  const handleSi = () => {
    if (deleteRow(registroSeleccionado.id, '/update_lugar_atencion/')) {
      toast.success(t('Record deleted successfully!'))
      router.push('./lugar-atencion')
    } else {
  ...
  const schema = yup.object().shape({
    ...
    ubicacion: yup
      .string()
      .min(1, obj => showErrors(t('location'), obj.value.length, obj.min))
      .required(),
    estado: yup.string()
  })
  ...
  const onSubmit = data => {
    if (saveRow(data, '/store_lugar_atencion/')) {
  ...
  const onSubmitEdit = e => {
    e.preventDefault()
    if (saveRow(registroSeleccionado, '/store_lugar_atencion/')) {
