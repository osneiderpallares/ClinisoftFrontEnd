...
const endPoint = 'http://127.0.0.1:8000/show_modalidad_atencion/'
...
const AppPage = ({}) => {
  ...
  const handleSi = () => {
    if (deleteRow(registroSeleccionado.id, '/update_modalidad_atencion/')) {
      toast.success(t('Record deleted successfully!'))
      router.push('./modalidad-atencion')
    } else {
  ...
  const onSubmit = data => {
    if (saveRow(data, '/store_modalidad_atencion/')) {
  ...
  const onSubmitEdit = e => {
    e.preventDefault()
    if (saveRow(registroSeleccionado, '/store_modalidad_atencion/')) {
