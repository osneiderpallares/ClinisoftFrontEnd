...
const endPoint = 'http://127.0.0.1:8000/show_tipo_cita/'
const endPoint_tcg = 'http://127.0.0.1:8000/show_tipo_cita_grupo/'
...
const AppPage = ({}) => {
  ...
  const columns = [
    ...
    },
    {
      flex: 0.15,
      minWidth: 120,
      field: 'nombre_tipos_citas_grupo',
      headerName: t('Appointment Types Group')
    },
    {
      field: 'facturable',
  ...
  const [tipocitagrupoRows, settipocitagrupoRows] = useState(null)

  const peticionGet = async () => {
    await axios.get(endPoint).then(response => {
      setRows(response.data)
    })
    await axios.get(endPoint_tcg).then(response => {
      settipocitagrupoRows(response.data)
    })
  }
  ...
  const handleSi = () => {
    if (deleteRow(registroSeleccionado.id, '/update_tipo_cita/')) {
      toast.success(t('Record deleted successfully!'))
      router.push('./tipo-cita')
    } else {
  ...
  const schema = yup.object().shape({
    ...
    abreviacion: yup
      ...
      .required(),
    tipos_citas_grupo: yup
      .string()
      .min(1, obj => showErrors(t('appointment types group'), obj.value.length, obj.min))
      .required(),
    facturable: yup
      .string()
      .min(1, obj => showErrors(t('billable'), obj.value.length, obj.min))
      .required(),
    estado: yup.string()
  })
  ...
  const onSubmit = data => {
    if (saveRow(data, '/store_tipo_cita/')) {
  ...
  const onSubmitEdit = e => {
    e.preventDefault()
    if (saveRow(registroSeleccionado, '/store_tipo_cita/')) {
  ...
  const totalPages = Math.ceil((totalfilas) / paginationModel.pageSize)
  const table = (
    ...
    <DataGrid
      ...
      columnVisibilityModel={{ create_up: false }}
      slotProps={{
        baseButton: {
          size: 'medium',
          variant: 'outlined'
                ...
                <Controller
                  ...
                  render={({ field: { value, onChange } }) => (
                    <CustomTextField
                      ...
                      fullWidth
                      value={value}
                      label={t('Appointment Types Group')}
                      SelectProps={{
                        MenuProps,
                        value: value,
                        onChange:onChange
                      }}
                      error={Boolean(errors.tipos_citas_grupo)}
                      aria-describedby='validation-schema-appointment types group'
                      {...(errors.tipos_citas_grupo && { helperText: errors.tipos_citas_grupo.message })}
                      >
                      {tipocitagrupoRows?.map(tcg => {
                        return (
                ...
                <Controller
                  ...
                  render={({ field: { value, onChange } }) => (
                    <CustomTextField
                      ...
                      error={Boolean(errors.facturable)}
                      aria-describedby='validation-schema-billable'
                      {...(errors.facturable && { helperText: errors.facturable.message })}
                    >
            ...
            <Grid container spacing={6}>
              <input
                ...
                onChange={e => {
                  setRegistroSeleccionado({
                    ...
                    tipos_citas_grupo: registroSeleccionado.tipos_citas_grupo,
                    facturable: registroSeleccionado.facturable,
              ...
              <Grid item sm={6} xs={6}>
                <CustomTextField
                  ...
                  label={t('Appointment Types Group')}
                  SelectProps={{
                    value:registroSeleccionado.tipos_citas_grupo,
                    onChange: e => {
                      setRegistroSeleccionado({
                        id: registroSeleccionado.id,
                        nombre: registroSeleccionado.nombre,
                        abreviacion: registroSeleccionado.abreviacion, 
                        tipos_citas_grupo: e.target.value,
                        facturable: registroSeleccionado.facturable,
                        estado: registroSeleccionado.estado
                    })
                   }
              ...
              <CustomTextField
                ...
                label={t('Billable')}
                SelectProps={{
                  value: registroSeleccionado.facturable,
                  onChange: e => {
                    setRegistroSeleccionado({
                      id: registroSeleccionado.id,
                      nombre: registroSeleccionado.nombre,
