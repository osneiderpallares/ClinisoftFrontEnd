...
const endPoint = 'http://127.0.0.1:8000/show_tipo_consulta/'
...
const AppPage = ({}) => {
  ...
  const handleSi = () => {
    if (deleteRow(registroSeleccionado.id, '/update_tipo_consulta/')) {
      toast.success(t('Record deleted successfully!'))
      router.push('./tipo-consulta')
    } else {
  ...
  const schema = yup.object().shape({
    ...
    requiere_nota: yup
      .string()
      .min(1, obj => showErrors(t('requires note'), obj.value.length, obj.min))
      .required(),
    estado: yup.string()
  })
  ...
  const onSubmit = data => {
    if (saveRow(data, '/store_tipo_consulta/')) {
  ...
  const onSubmitEdit = e => {
    e.preventDefault()
    if (saveRow(registroSeleccionado, '/store_tipo_consulta/')) {
                ...
                <Controller
                  ...
                  render={({ field: { value, onChange } }) => (
                    <CustomTextField
                      ...
                      fullWidth
                      value={value}
                      label={t('Requires Note')}
                      SelectProps={{
                        value:value,
                        onChange:onChange
                    }}
                      error={Boolean(errors.requiere_nota)}
                      aria-describedby='validation-schema-requires note'
                      {...(errors.requiere_nota && { helperText: errors.requiere_nota.message })}
                      >
            ...
            <Grid container spacing={6}>
              <input
                ...
                onChange={e => {
                  setRegistroSeleccionado({
                    ...
                    requiere_nota: registroSeleccionado.requiere_nota,
              ...
              {/* <Grid item sm={2} xs={2}>
                <CustomTextField
                  ...
                  label={t('Requires Note')}
                  SelectProps={{
                    value:registroSeleccionado.requiere_nota,
                    onChange:e => {
                     setRegistroSeleccionado({
                        id: registroSeleccionado.id,
                        nombre: registroSeleccionado.nombre,
                        requiere_nota: e.target.value,
                        estado: registroSeleccionado.estado
                     })
                    }
                  }}
              ...
              <Grid item sm={12} xs={2}>
                {t('Requires Note')}
                <Checkbox                
                checked={registroSeleccionado.requiere_nota}
                onChange={e => {
                    setRegistroSeleccionado({
                    ...registroSeleccionado,
                    id: registroSeleccionado.id,
                    nombre:registroSeleccionado.nombre,
                    requiere_nota: e.target.checked,
                    estado: registroSeleccionado.estado
                    });
                }}
