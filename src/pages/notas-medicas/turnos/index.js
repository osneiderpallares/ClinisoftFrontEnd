...
const endPoint = 'http://127.0.0.1:8000/show_turno/'
...
const AppPage = ({}) => {
  ...
  const handleSi = () => {
    if (deleteRow(registroSeleccionado.id, '/update_indice/')) {
      toast.success(t('Record deleted successfully!'))
      router.push('./grupo-indice')
    } else {
  ...
  const onSubmitEdit = e => {
    e.preventDefault()
    if (saveRow(registroSeleccionado, '/store_indice/')) {
  ...
  const currentLocaleText =i18n.language === 'es' ? esES.components.MuiDataGrid.defaultProps.localeText : enUS.components.MuiDataGrid.defaultProps.localeText;
    ...
    <DataGrid
      ...
      />
       <div style={{ display: 'flex', justifyContent: 'center', marginTop: '0px' }}>
        ...
        <Pagination count={totalPages} color="primary"  paginationModel={paginationModel.page} showFirstButton showLastButton  onChange={handlePageChange} />
       </div>
