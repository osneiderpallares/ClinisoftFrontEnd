...
const endPoint = 'http://127.0.0.1:8000/show_via_ingreso/'
...
const AppPage = ({}) => {
  ...
  const handleSi = () => {
    if (deleteRow(registroSeleccionado.id, '/update_via_ingreso/')) {
      toast.success(t('Record deleted successfully!'))
      router.push('./vias-ingreso')
    } else {
  ...
  const onSubmit = data => {
    if (saveRow(data, '/store_via_ingreso/')) {
  ...
  const onSubmitEdit = e => {
    e.preventDefault()
    if (saveRow(registroSeleccionado, '/store_via_ingreso/')) {
