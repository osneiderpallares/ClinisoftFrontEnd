...
const endPoint = 'http://127.0.0.1:8000/show_especialidad/'
...
const AppPage = ({}) => {
  ...
  const handleSi = () => {
    if (deleteRow(registroSeleccionado.id, '/update_especialidad/')) {
  ...
  const onSubmit = data => {
    if (saveRow(data, '/store_especialidad/')) {
  ...
  const onSubmitEdit = e => {
    e.preventDefault()
    if (saveRow(registroSeleccionado, '/store_especialidad/')) {
  ...
  const table = (
    <DataGrid
      ...
      columnVisibilityModel={{ id: false, create_up: false, cita: false }}
      slotProps={{
        baseButton: {
          size: 'medium',
          variant: 'outlined'
            ...
            <Grid container spacing={6}>
              <input
                ...
                onChange={e => {
                  setRegistroSeleccionado({
                    ...
                    cita: registroSeleccionado.cita,
              ...
              <Grid item sm={6} xs={6}>
                <FormControlLabel
                  control={
                    <Switch
                      checked={registroSeleccionado.cita}
                      onChange={e => {
                        setRegistroSeleccionado({
                          id: registroSeleccionado.id,
                          nombre: registroSeleccionado.nombre,
                          cita: e.target.checked,
                          estado: registroSeleccionado.estado
                        })
                      }}
                    />
