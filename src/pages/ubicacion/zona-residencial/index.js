...
const endPoint = 'http://127.0.0.1:8000/show_zona_residencial/'
...
const AppPage = ({}) => {
  ...
  const handleSi = () => {
    if (deleteRow(registroSeleccionado.id, '/update_zona_residencial/')) {
      toast.success(t('Record deleted successfully!'))
      router.push('./zona-residencial')
    } else {
  ...
  const onSubmit = data => {
    if (saveRow(data, '/store_zona_residencial/')) {
  ...
  const onSubmitEdit = e => {
    e.preventDefault()
    if (saveRow(registroSeleccionado, '/store_zona_residencial/')) {
